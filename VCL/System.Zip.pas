...
{$ENDIF}
    ...
    if FileExists(LFileName) then
      begin
        TFile.SetCreationTime(LFileName, FileDateToDateTime(FFiles[Index].ModifiedDateTime));
        TFile.SetLastWriteTime(LFileName, FileDateToDateTime(FFiles[Index].ModifiedDateTime));
        if Hi(FFiles[Index].MadeByVersion) = 0 then // DOS
          TFile.SetAttributes(LFileName, TFile.IntegerToFileAttributes(FFiles[Index].ExternalAttributes and (faReadOnly or faHidden or faSysFile or faArchive)));
      end;
  finally
    LInStream.Free;
  end;
...
begin
  ...
  try
    ...
    LHeader.ExternalAttributes := UINT32(TFile.FileAttributesToInteger(TFile.GetAttributes(FileName)) and (faReadOnly or faHidden or faSysFile or faArchive));
    if ArchiveFileName <> '' then
...
procedure TZipFile.Delete(const Index: Integer);
var
  i, j: Integer;
