...
uses System.RTLConsts, System.Types, Data.DBConsts, System.Generics.Defaults
{$IFDEF MACOS}
...
begin
  ...
  begin
    ...
    if not AAutomaticOnly or (eLiveCycle = lcAutomatic) then begin
...
begin
  if Value = [lcPersistent] then begin
    eCycle := lcPersistent;
    FAutomaticFieldsCount := 0;
  end
  else if Value = [lcAutomatic] then begin
    eCycle := lcAutomatic;
...
var
  S: string;
begin
  if GetValue(S) then Result := string(S) else Result := '';
end;
...
begin
  ...
  if GetData(F) then Result := TBitConverter.ToSingle(F) else Result := 0;
end;
...
begin
  ...
  if GetData(F) then Result := TBitConverter.ToExtended(F) else Result := 0;
end;
...
begin
  SetData(BytesOf(@Value, SizeOf(TSQLTimeStamp)), True);
end;
...
begin
  ...
  SetData(BytesOf(@Value, SizeOf(TBcd)), True);
end;
...
begin
  ...
  SetData(BytesOf(@LValue, SizeOf(TBcd)), True);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if not (Field.DataType in ftCalcFieldTypes) then
