...
{ TCustomADODataSet }
  ...
  published
    ...
    // При True вызывается оптимизированный метод для Profit!!!
    property ProfitOptimized: Boolean read FProfitOptimized write FProfitOptimized;
  end;
...
{ новый код без избыточных синхронизаций и проверки положения курсора}
{ еще новее отдельно обрабатывается null}
begin
  if ProfitOptimized then
    begin
  ...
  if Result then
    ...
    begin
      // Это стандартный код Embarcadero до изменения под Profit!!!
...
begin
  ...
  begin
    ...
    begin
      ...
      CursorPosChanged;
      //Result := Recordset.AbsolutePosition; {Так было}
      Result := Recordset.Bookmark; {Так стало}
    end;
  end;
end;
