...
type
  ...
  protected
    ...
    FPort                   : Byte; // COM Port to use (1..8)
  ...
  published
    ...
    property Port: Byte read FPort write SetPort default 2;
...
begin
  ...
  FPort                   := 2;        // COM 2
  FPortBaudRate           := br9600;   // 9600 bauds
...
//------------------------------------------------------------------------------
procedure TCommPortDriver.SetPort( Value: Byte );
begin
  if not Connected then
  begin
    if Value<1  then FPort:=1  else
    if Value>99 then FPort:=99 else FPort := Value;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  ...
  comName[3] := chr( ord('0') + FPort );
  comName[4] := #0;
  FPortHandle := CreateFile( comName,
...
//------------------------------------------------------------------------------
function TCommPortDriver.GetState: Integer;
var flag:Cardinal;
begin
    ...
    if ReadFile( FPortHandle,
      ...
      begin
        ...
        then begin FPortReceiveData( Self, FInBuffer, nRead ); end
                   ...
                   begin if Byte((Pointer(NativeInt(FInBuffer)+i))^)= FEndByte
                         then begin FPortReceiveData( Self, FInBuffer, i+1 );
                                    for j:=0 to (FOffsetBuffer-(i+1))-1 do
                                      Byte((Pointer(NativeInt(FInBuffer)+j    ))^):=
                                      Byte((Pointer(NativeInt(FInBuffer)+j+i+1))^);
...
procedure Register;
begin
  RegisterComponents('Profit', [TCommPortDriver]);
end;
